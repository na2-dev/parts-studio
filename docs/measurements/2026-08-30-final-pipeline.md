...
## 注記（2026-08-31）: 1〜7 すべてが1コマンドになった

**下の手順はもう手で打たない。** `tools/run_pipeline.py` が 1〜7 をまとめて行う
（[実測](2026-08-31-run-pipeline.md)。4 枚 → 完成 glb が 436 秒）。
手順3〜7だけなら `tools/make_parts.py`（[実測](2026-08-31-up-axis.md)）。

**下の「約3分 / 約30秒」という見積もりも古い。** 実測は 110 秒 / 15 秒 / 190 秒。

あわせて上方向の扱いを直しているので、**下のコマンドをそのまま打つと結果が悪くなる**。

- 塗りへは Y 上に直して渡す（Z 上のままだと顔が頭の裏側に付く）
- パーツの上方向は全身から引き継ぐ（パーツからは測れない）
- 出来上がりは glTF の Y 上にする（そうしないと標準のビューアで寝る）
- 頭をならすとき首の切り口は固定する（体と合わなくなる）
