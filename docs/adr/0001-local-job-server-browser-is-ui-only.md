...
> **追記（2026-09-03・[#7](https://github.com/na2-dev/parts-studio/issues/7)）**:
> ジョブサーバーを実装した（`tools/job_server.py`）。標準ライブラリだけで動き、
> Job は `run_pipeline.py` を別プロセスで実行する。API と作りの要点は
> [docs/setup/job-server.md](../setup/job-server.md)。
