...
VRAM は README に「24GB 以上が必要」とあるが、パイプラインに `low_vram` 経路が実装されており、
issue #4 に「RTX 4070（16GB）＋システムRAM 128GB で動作」という報告がある。ただし同じ議論で
「low_vram でも大量のシステムRAMが要る（初回ロードで約 28GB、生成中は CPU と GPU を行き来）」
とも報告されており、**効くのは VRAM だけでなくシステムRAM である**。
**手元の RTX 4070 Ti SUPER（16GB）での実動はまだ確かめていない。**
