...
KNOWN_PARAMS = {
    'res': lambda v: isinstance(v, int) and v >= 1024 and (v - 1024) % 128 == 0,
    'seed': lambda v: isinstance(v, int),
    'texsize': lambda v: isinstance(v, int) and v >= 1,
    'margin': lambda v: isinstance(v, (int, float)) and -0.5 < v < 0.5,
    'voxel': lambda v: isinstance(v, (int, float)) and 0.001 <= v <= 0.1,
...
class Runner(threading.Thread):
    ...
    def run(self):
        while True:
            ...
            if job is None:
                continue
            if job.cancel_requested:        # 待っている間に取り消された
    ...
    def run_one(self, job):
        ...
        try:
            ...
            job.set(state='failed', error=f'{type(e).__name__}: {e}',
...
def cancel(job):
    """取り消す。実行中ならプロセスを止める。もう終わっていれば False。"""
    with job.lock:
        ...
        proc = job.proc
    if proc is not None:
        proc.kill()                         # ★kill。生成の途中に「きれいな止め方」は無い
    else:
        job.set(state='canceled', message='取り消されました', finished=time.time())
    return True
